...
w_wsd_data<-w.wsd("M1004900,000985.CSI,037.CS,801180.SI","close","2013-01-01","2017-12-31","PriceAdj=F")
...
meanReturns[1]<-1.0375^(1/220)-1#moneytary
meanReturns[2]<-1.060^(1/220)-1#stock
meanReturns[3]<-1.041^(1/220)-1#bond
...
port <- add.constraint(port, type = "box", min = c(0.07,0.05,0.50427,0), max = c(1,1,1,0.15)) ###4.76%~25%


# Leverage 仓位限制，full要求一直满仓。
port <- add.constraint(portfolio = port, type = "full_investment")

port <- add.constraint(portfolio = port, type = "return", return_target=0.00000,enabled = FALSE)#其实不发挥作用
...
maxret.port <- add.objective(port, type = "return", name = "mean")#这句可能有问题,could be "mean"
# Optimize
...
eff.frontier$Risk[which.max(eff.frontier$Sharperatio)-5]*sqrt(220)

(1+eff.frontier$Return[which.max(eff.frontier$Sharperatio)-5])^220

frontier.weights[which.max(eff.frontier$Sharperatio)-5,]*100

eff.frontier$Sharperatio[196]
